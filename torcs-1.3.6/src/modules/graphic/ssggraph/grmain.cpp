...
{
    grScreens[0]->switchMirror();
}

static void
grToggleVintLogging(void * /* dummy */)
{
	toggleVintLogging();
	const char* log_dir = getVintLogDirectory();
	if (isVintLoggingEnabled()) {
		GfOut("ViNT Data Logging ENABLED - Data saved to: %s\n", log_dir);
	} else {
		GfOut("ViNT Data Logging DISABLED\n");
	}
}
...
initView(int x, int y, int width, int height, int /* flag */, void *screen)
{
	...
	GfuiAddKey(screen, 'l',            "Toggle ViNT Logging", (void*)0, grToggleVintLogging, NULL);
...
initTrack(tTrack *track)
{
	...
	// Initialize ViNT data logger with track name
	initVintDataLogger(track->name);
...
shutdownTrack(void)
{
	...
	// Cleanup ViNT data logger
	cleanupVintDataLogger();
