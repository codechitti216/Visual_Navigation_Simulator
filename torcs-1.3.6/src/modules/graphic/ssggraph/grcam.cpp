...
class cGrCarCamInsideFixedCar : public cGrPerspCamera
{
 public:
	...
	// Roof-mounted camera position: on top of the car
	// Position camera on the roof, looking down at the road ahead
	p[0] = car->_bonnetPos_x;        // Same longitudinal position as bonnet
	p[1] = car->_bonnetPos_y;        // Same lateral position as bonnet
	p[2] = car->_bonnetPos_z + 1.2;  // 1.2m higher than bonnet (roof level)
	sgXformPnt3(p, car->_posMat);
	...
	// Look forward horizontally from roof position (like Waymo car)
	P[0] = car->_bonnetPos_x + 30.0;  // Look 30m ahead
	P[1] = car->_bonnetPos_y;
	P[2] = car->_bonnetPos_z;         // Look at road level, not down from roof
	sgXformPnt3(P, car->_posMat);
